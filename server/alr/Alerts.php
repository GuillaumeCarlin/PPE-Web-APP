...
switch ($action) {
	case 'alertLst':
		...
		$aListe = $Bdd->QryToArray(sprintf($aAlertsQRY['list'], $Bdd->FormatSql($aSpecFilter['sab_id'], 'C')));
		$bSucces = (count($aListe) > 0);
		break;

	case 'alertDetail':
		$aListe = $Bdd->QryToArray(sprintf($aAlertsQRY['detail'], $alr_id));
		$bSucces = (count($aListe) > 0);
		break;

	case 'alertUpdate':
		$Bdd->QryExec(sprintf($aAlertsQRY['update'], $alr_id, $rsc_id, $Bdd->FormatSql($commentaire, 'C'), $terminer));
