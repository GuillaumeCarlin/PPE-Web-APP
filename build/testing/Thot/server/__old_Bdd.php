...
Class GestBdd {
	...
	var $NomBase = "";
	var $ConvUtf8 = false;
	var $TblSsnVielles = array();
	var $NbreHresLimite = 12;
	var $ForcerEspion = false;
	var $aTypeChamp = array();
	var $aExecReq = array("success" => false, "errno" => 0, "message" => '');
	var $ForcerDroit = false;
	var $bDebug = false;
	var $sFicLog = "Bdd.log";
	var $sRacine = "";
	var $bSilent = false;
	 ...
	 */
	function GestBdd($aBase) {
		require("RequetesSys.php");
	...
	function Connexion() {
		...
		switch ($this->PrmBase["typebase"]) {
			case "MySql" :
				$this->CnxDb = mysql_connect($this->PrmBase["host"], $this->PrmBase["username"], $this->PrmBase["passe"]);

				if (!$this->CnxDb) {
					$Retour = false;
				}

				$this->IdDb = mysql_select_db($this->PrmBase["nombase"], $this->CnxDb);
				if (!$this->IdDb) {
					$Retour = false;
				}

				if ($Retour) {
					//mysql_query("SET NAMES latin1",$this->CnxDb);
					//mysql_query("SET sql_mode='ANSI_QUOTES'");

					/*
					  $this->Espion("-------------- Avant SET NAMES utf8 ----------------");
					  //$TblSVL=$this->QryToArray("SHOW VARIABLES LIKE 'char%'",$this->CnxDb);
					  $TblSVL=$this->QryToArray("SELECT ORD( 'é' ) AS Ord, COLLATION( 'é' ) AS Coll",$this->CnxDb);
					  for ($IndEsp=0;$IndEsp<count($TblSVL);$IndEsp++) {
					  $this->Espion($TblSVL[$IndEsp]["ORD"]." - ".$TblSVL[$IndEsp]["COLL"]);
					  }
					 */

					mysql_query("SET NAMES utf8", $this->CnxDb);

					/*
					  $this->Espion("-------------- Apres SET NAMES utf8 ----------------");
					  //$TblSVL=$this->QryToArray("SHOW VARIABLES LIKE 'char%'",$this->CnxDb);
					  $TblSVL=$this->QryToArray("SELECT ORD( 'é' ) AS Ord, COLLATION( 'é' ) AS Coll",$this->CnxDb);
					  for ($IndEsp=0;$IndEsp<count($TblSVL);$IndEsp++) {
					  $this->Espion($TblSVL[$IndEsp]["ORD"]." - ".$TblSVL[$IndEsp]["COLL"]);
			...
			case "SqlServer" :
				$ParamSqlServer = true;
				$ConnectionInfo = array("Database" => $this->PrmBase["nombase"], 'ReturnDatesAsStrings' => true);
				$this->CnxDb = sqlsrv_connect($this->PrmBase["host"], $ConnectionInfo);
				if (!$this->CnxDb) {
					$Retour = false;
				}
				break;
			...
			case "Old_MsSql" :
				...
				$ParamSqlServer = true;
				$this->CnxDb = mssql_connect($this->PrmBase["host"], $this->PrmBase["username"], $this->PrmBase["passe"]);

				if (!$this->CnxDb) {
					$Retour = false;
				}

				mssql_select_db($this->PrmBase["nombase"], $this->CnxDb);
				break;
			...
			case "MsSql":
				...
				switch (PHP_OS) {
					case 'Linux':
						$this->CnxDb = new pdo(
								'dblib:host=' . $this->PrmBase["host"] . ';dbname=' . $this->PrmBase["nombase"], $this->PrmBase["username"], $this->PrmBase["passe"]
						);
						break;
					default:
						$this->CnxDb = new pdo(
								'sqlsrv:server=' . $this->PrmBase["host"] . ';Database=' . $this->PrmBase["nombase"], $this->PrmBase["username"], $this->PrmBase["passe"]
						);
						break;
				}
			...
			case "Odbc" :
				$ParamSqlServer = true;
				$this->CnxDb = odbc_connect($this->PrmBase["host"], $this->PrmBase["username"], $this->PrmBase["passe"]);

				if (!$this->CnxDb) {
					$Retour = false;
					$this->Erreur("Erreur à la connexion ODBC (" . $this->PrmBase["host"] . ") : " . odbc_errormsg());
				}
				break;
		}
	 ...
	 * @version JJ/MM/AA edblv RND#ND-ND.ND Création
	 */
	function FormatSql($val, $format, $type = "M") {
		$Zero = false;
		if ($format === "Z") {
			$format = "N";
			$Zero = true;
			...
			case "DJC" :
				if ($type == "M") {
					...
					if ($val == "") {
						...
						$tablo = explode("/", $val);
						$chaine = gregoriantojd(intval($tablo[1]), // Mois
								intval($tablo[0]), // Jour
								intval($tablo[2])); // Année
					}
				}
				else {
					// Lecture
	...
	function MajEnreg(&$oRecord) { //}$sTable, $aChamps, $sIdent, &$iIdEnreg) {
		...
		if (intval($oRecord->idrecord) > 0) {
			...
			//$this->Erreur(sprintf($sReq,$sTable, $sUpdate,$sIdent,$this->FormatSql($iIdEnreg, $aStruc[$sIdent]["typechamp"])));
	...
	function update4D($aRecord) {
		...
		try {
			...
			$clientSOAP = new SoapClient(
					$_SESSION[$_SESSION['AppName']]["PARAMETRES"]['soap']["client"], $aParam4D
			);
	...
	function Erreur($Chaine, $bDetail = true) {
	 ...
	 * @version JJ/MM/AA edblv RND#ND-ND.ND Création
	 */
	function QryExec($Requete) {
		$initialEncoding = mb_detect_encoding($Requete);

		switch ($this->PrmBase["typebase"]) {
			case "MySql" :
				$Result = mysql_query(utf8_encode($Requete), $this->CnxDb);
				$Retour = null;
				...
				if ($Result > 0) {
					...
					$Retour[0] = false;
					$Retour[1] = mysql_error();
					$Retour[2] = mysql_errno();

					$this->Erreur("QryToArray : " . $Requete);
					$this->Erreur($Result . " : " . mysql_error());
				}
				break;
			...
			case "Old__MsSql" :
				...
				try {
					if ($initialEncoding != "UTF-8") {
						$Result = mssql_query(utf8_encode($Requete), $this->CnxDb);
					}
					else {
						$Result = mssql_query($Requete, $this->CnxDb);
					}
				...
				if ($bResult) {
					...
					if ($Result > 0) {
						...
						$Retour[0] = false;
						$Retour[1] = mssql_get_last_message();
						$Retour[2] = 0;

						$this->Erreur("QryToArray : " . $Requete);
						$this->Erreur($Result . " : (0) " . mssql_get_last_message());
					}
				}
				break;
			...
			case "MsSql" :
				//---- MsSql avec PDO
				$oQry = $this->CnxDb->prepare($Requete);
				$oQry->execute();
				$aErr = $oQry->errorInfo();
				$Retour = null;
			...
			case "Odbc" :
				$Result = odbc_exec($this->CnxDb, $Requete);
				$Retour = null;
				...
				if ($Result > 0) {
					...
					$Retour[0] = false;
					$Retour[1] = odbc_errormsg();
					$Retour[2] = odbc_error();

					$this->Erreur("QryToArray : " . $Requete);
					$this->Erreur($Result . " : (" . odbc_error() . ") " . odbc_errormsg());
				}
				break;
		}
	 ...
	 * @version JJ/MM/AA edblv RND#ND-ND.ND Création
	 */
	function qryDebug($sQuery, array $aParams = []) {
		$aKeys = array();
		$aValues = array();
	 ...
	 * @version JJ/MM/AA edblv RND#ND-ND.ND Création
	 */
	function StructureTbl($sNomTable) {
	...
	function QryToArray($Requete, $sIndice = "", $bDebug = false) {
		...
		switch ($this->PrmBase["typebase"]) {
			/*
			case "MySql" :
				$Result = mysql_query($Requete, $this->CnxDb);
				$aDatas = null;
				...
				if ($Result > 0) {
					...
					while ($Enreg = mysql_fetch_row($Result)) {
						...
						for ($IndCol = 0; $IndCol < count($Enreg); $IndCol++) {
							$NomChamp = strtoupper(mysql_field_name($Result, $IndCol));
							$ValChamp = $Enreg[$IndCol];
							$this->aTypeChamp[$NomChamp] = mysql_field_type($Result, $IndCol);

							switch (mysql_field_type($Result, $IndCol)) {
								case "date" :
									$ValChamp = $this->FormatSql($ValChamp, "DSQL", "L");
									break;
								default :
									break;
							}

							$TblLigne[$NomChamp] = $ValChamp;
						}

						$aDatas[$IndLig] = $TblLigne;
						$IndLig++;
					}
				}
				else {
					$this->Espion("QryToArray : " . $Requete);
					$this->Espion($Result . " : " . mysql_error());
				}
				break;
			*/

			/*
			case "Old__MsSql" : // (FreeTDS)
				...
				if ($Result > 0) {
					...
					while ($Ligne = mssql_fetch_row($Result)) {
						...
						for ($IndCol = 0; $IndCol < mssql_num_fields($Result); $IndCol++) {
							$NomChamp = strtolower(mssql_field_name($Result, $IndCol));
							$ValChamp = $Ligne[$IndCol];
							$this->aTypeChamp[$NomChamp] = mssql_field_type($Result, $IndCol);

							switch (mssql_field_type($Result, $IndCol)) {
								case "datetime" :
									$ValChamp = $this->FormatSql($ValChamp, "DSQL", "L");
									break;

								case "char" :
									if (mssql_field_length($Result, $IndCol) == 64) {
										//---- Si le champ est de type char ET qu'il a une longueur de 64
										// Il y a de très fortes chances pour que ce soit un champ time
										// Malheureusement mssql_field_length ne fait pas la différence
										// entre un champ time et un champ varchar
										$ValChamp = $this->FormatSql($ValChamp, "HM", "L");
									}
									else {
							...
							$TblLigne[$NomChamp] = $ValChamp; //utf8_encode($ValChamp);
						}

						if ($sIndice <> "") {
							$sValIndice = $TblLigne[$sIndice];
							$aDatas[$sValIndice] = $TblLigne;
						}
						else {
							$aDatas[$IndLig] = $TblLigne;
						}
					...
					$this->ExecReq[0] = false;
					$this->ExecReq[1] = mssql_get_last_message();
					$this->ExecReq[2] = 0;

					$this->Erreur("QryToArray : " . $Requete . "\nErreur ->" . $Result . " : " . mssql_get_last_message());
				}
				break;
			*/
			...
			case "Odbc" :
				...
				if ($Result > 0) {
					...
					while (odbc_fetch_row($Result)) {
						...
						for ($IndCol = 1; $IndCol <= odbc_num_fields($Result); $IndCol++) {
							$NomChamp = strtoupper(odbc_field_name($Result, $IndCol));
							$ValChamp = odbc_result($Result, $NomChamp);
							$this->aTypeChamp[$NomChamp] = odbc_field_type($Result, $IndCol);

							switch (odbc_field_type($Result, $IndCol)) {
								case "datetime" :
									$ValChamp = $this->FormatSql($ValChamp, "DSQL", "L");
									break;
								case "time" :
							...
							$TblLigne[$NomChamp] = utf8_encode($ValChamp);
						}
					...
					$this->Erreur("QryToArray : " . $Requete);
					$this->Erreur($Result . " : " . odbc_errormsg());
				}
				break;
			*/
		}
	...
	function DernierEnreg($sTableName) {
		$IdEnreg = 0;
	...
	function InfosFormulaire($aForm) {
		$aListeForm = array();
		$sFiltre = "";
	...
	function ChargeParametres($bEnSession = true) {
		$aParametres = array();
	...
	function SauveParametre($sCodeParam, $sCodeDetail, $sValeur) {
	...
	function ChargePreferences($aPrefUti) {
	 ...
	 * @version JJ/MM/AA edblv RND#ND-ND.ND Création
	 */
	function validPath($sPath, $sDest = 'w') {
		$sSlash = '\\';
	 ...
	 * @version JJ/MM/AA edblv RND#ND-ND.ND Création
	 */
	function dir($sPath, $sJocker = '*.*', $aReplacePath = ['', '']) {
		$aFiles = [];
		$aDirectories = [];
		...
		*/
		...
		while (false !== ($sItem = $oDir->read())) {
			if (is_dir($sPath . $sItem)) {
				...
				$aFiles[] = ['chemin' => $sFilePath, 'fichier' => utf8_encode($sItem)];
			}
		}
	...
	function Begin() {
		switch ($this->PrmBase["typebase"]) {
			case "MySql" :
				mysql_query("BEGIN", $this->CnxDb);
				break;

			case "SqlServer" :
				sqlsrv_begin_transaction($this->CnxDb);
				break;
			...
			case "Odbc" :
				odbc_autocommit($this->CnxDb, FALSE);
				break;
		}
	}
	...
	function Rollback() {
		switch ($this->PrmBase["typebase"]) {
			case "MySql" :
				mysql_query("ROLLBACK", $this->CnxDb);
				break;

			case "SqlServer" :
				sqlsrv_commit($this->CnxDb);
				break;
	...
	function Commit() {
		switch ($this->PrmBase["typebase"]) {
			case "MySql" :
				mysql_query("COMMIT", $this->CnxDb);
				break;

			case "SqlServer" :
				sqlsrv_rollback($this->CnxDb);
				break;
...
//$this->Erreur("Filtre : " . $Filtre);
	...
	function TblIndiceCol($Tableau, $Colonne) {
		$TblResult = array();
	...
	function ChargePanier($IdFormulaire, $NomChamp, $IdPanier) {
	...
	function AjoutPanier($IdFormulaire, $NomChamp, $IdPanier, $LstId) {
		//---- Test l'existence de la variable PANIER
		if (!isset($_SESSION[$_SESSION['AppName']]["PANIER"])) {
			$_SESSION[$_SESSION['AppName']]["PANIER"] = array();
		}
	...
	function InfosPanier($IdFormulaire, $NomChamp) {
		...
		if (isset($_SESSION[$_SESSION['AppName']]["PANIER"])) {
			if (isset($_SESSION[$_SESSION['AppName']]["PANIER"][$PanierForm])) {
				if (isset($_SESSION[$_SESSION['AppName']]["PANIER"][$PanierForm][$NomChamp])) {
					$TblInfosPanier[0] = array("IDPANIER" => $_SESSION[$_SESSION['AppName']]["PANIER"][$PanierForm][$NomChamp]["IDPANIER"],
	...
	function AbandonPanier($IdFormulaire, $NomChamp) {
		$PanierForm = "Form" . $IdFormulaire;
	...
	function AjoutItemVide($aListeOrig) {
		$aListeRetour = array();
	...
	function TblToXML($DocXml, $sRacine, $NomNoeud, $TblDonnees, $TblAttr = array(), $Valeur = "", $Niveau = 0) {
	...
	function AjoutNoeud($DocXml, $NomNoeud, $TblAttr = array(), $Valeur = "") {
		if ($Valeur != "") {
	...
	function TblToExtJS($oDocXml, $oRacine, $sNomNoeud, $aDonnees, $sNomId, $aTotaux = array("TotEnreg" => 0, "TotPage" => 0)) {
	...
	function TblToChart($DocXml, $sRacine, $NomNoeud, $TblDonnees) {
		//---- On parcours le recordset
		for ($IndDon = 0; $IndDon < count($TblDonnees); $IndDon++) {
			$ValNoeud = "";
			for (reset($TblDonnees[$IndDon]); $NomChm = key($TblDonnees[$IndDon]); next($TblDonnees[$IndDon])) {
	...
	function TblToGrid($DocXml, $sRacine, $TblDonnees, $NomId, $aUserData = array()) {
		$IdNull = 1;
		//---- On parcours le recordset
		for ($IndDon = 0; $IndDon < count($TblDonnees); $IndDon++) {
			//---- On répartit les colonnes entre $aAttributs et $TblUsrDat
	...
	function TblToAgenda($DocXml, $sRacine, $TblDonnees, $NomId) {
		//---- On parcours le recordset
		for ($IndDon = 0; $IndDon < count($TblDonnees); $IndDon++) {
			for (reset($TblDonnees[$IndDon]); $NomChm = key($TblDonnees[$IndDon]); next($TblDonnees[$IndDon])) {
	...
	function TblDdiToGantt($DocXml, $sRacine, $TblDonnees, $aProjet, $aTache) {
		$IdProjet = -1;
		$NoeudProjet = "";
	...
	function TblToCsv($FichierExport, $TblDonnees, $TblLibelle = array()) {
	...
	function TblToTree($DocXml, $sRacine, $TblDonnees, $NomLibelle, $Ident = "0", $Niveau = 1) {
		$IdentItem = "";
	...
	function TblToMenu($DocXml, $sRacine, $TblDonnees, $NomLibelle, $Ident = "0", $Niveau = 1) {
		$IdentItem = "";
		...
		for ($IndDon = 0; $IndDon < count($TblDonnees); $IndDon++) {
			...
			$aAttributsNoeud = array("id" => $IdentItem,
				"enabled" => $ItemActif);
	...
	function CreeRacine($Xml, $NomRacine, $Attribut = "", $Valeur = "") {
	...
	function StockeDonnees($NomTable4D, $NomChmIdent, $ValIdent, $TblNomChampsMaj, $TblValChampsMaj) {
		...
		$TblAttrTable = array("NOMTABLE" => $NomTable4D,
	...
	function SauveFic($Chaine, $NomFic) {
	...
	function ExtraitColonne($aTableau, $sColonne) {
		$aResult = array();
	...
	function Compteur($NomTable, $NomId) {
		$ValCompteur = 0;
	...
	function Verrou($NomTable, $IdEnreg) {
	...
	function SuppVerrou($NomTable, $IdEnreg) {
	...
	function DirString($sString, $sType = 'disk') {
		switch ($sType) {
			case "disk" :
	...
	function GenPanier($NomPanier, $IdFormulaire, $NomChamp, $ReqListe, $NomId) {
		$PanierForm = "Form" . $IdFormulaire;
...
#$this->SauvePanierDet($TblSauvePanier["IDPANIER"], $IdFormulaire, $NomChamp);
	...
	function TrouvePanier($NomPanier, $IdFormulaire, $NomChamp) {
		$PanierForm = "Form" . $IdFormulaire;
		$IdPanier = 0;
	...
	function SupprimePanier($IdPanier, $IdFormulaire = 0, $NomChamp = "") {
	...
	function SauvePanierTete($NomPanier, $IdFormulaire, $NomChamp, $Visibilite) {
		$PanierForm = "Form" . $IdFormulaire;
		$IdPanier = 0;
	...
	function SauvePanierDet($IdPanier, $IdFormulaire, $NomChamp) {
		$PanierForm = "Form" . $IdFormulaire;
	...
	function readDefault($sReqReadDefault, $sForm, $sFields) {
