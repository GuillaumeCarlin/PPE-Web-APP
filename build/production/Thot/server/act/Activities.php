...
switch ($action) {
	...
	case 'newCompAct': // création activité sur opération complémentaire
		// création ou identification de l'opération qualité à utiliser
		// c'est la procédure SQL qui fait ce travail
		$Bdd->QryExec(sprintf($aActivities['compactcreate'], $odf_id, $eqp_id, $opc_id ));
		...
		if (!$Bdd->aExecReq['success']) {
			...
			// on récupère les données de l'opération qui vient d'être créée
			$opn_id=$aListe['results'][0]['_newopnid'];
			$isnewopn=$aListe['results'][0]['_isnewop'];

			$iForce = 0;
			// on créé la nouvelle activité
			$Bdd->QryExec(sprintf($aActivities['create'], $opn_id, $odf_id, $org_id, $Bdd->FormatSql($rsc_id, 'C'), $iForce));
	...
	case 'LstActEC':
		error_log(sprintf($aActivities['sectioncurract'], $Bdd->FormatSql($aSpecFilter['sab_id'], 'C')));
