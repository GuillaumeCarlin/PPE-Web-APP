Ext.define('Thot.view.act.cor.FormRevisionReglageController', {
    extend: 'Ext.app.ViewController',
    alias: 'controller.act-cor-formrevisionreglage',
        ...
        Ext.Ajax.request({
            ...
            callback: function (opt, success, oResponse) {
                ...
                if (oBack.success) {
                     ...
                     */
                    oMe._loadOpnEqp(oBack.liste[0].opn_id, true);

                    /**
                     * mise à jour de la liste des opérations
                     */
                    oMe._loadOfOper(oBack.liste[0].odf_id, true);
     ...
     */
    _loadOfOper: function (sOfId, afterRender) {
        var oMe = this;
        var oForm = this.getView();
        var oOperationCbo = oForm.query('#opn_id')[0];
        var oOperationStr = oOperationCbo.getStore();
        var aFilter = [{ // on ne fournit pas le paramètre opn_estdisponible pour récupérer toutes les opérations
            type: 'odf_id',
            value: sOfId
        }];

        //---- Chargement des opérations liées à ce poste
     ...
     */
    _loadOpnEqp: function (sOpnId, afterRender) {
        var oForm = this.getView();
    ...
    /**
     * @author : edblv
     * @description Selection d'une opération dans la combo
        ...
        Ext.Ajax.request({
            ...
            callback: function (opt, success, oResponse) {
                ...
                if (oBack.success) {
                    ...
                    oMe._loadOpnEqp(oRecord.data.opn_id, false);
                } else {
                    var oMsg = Thot.app.MessageInfo();
                    oMsg.init(5000);
    ...
    onDateChange: function (oField) {
        ...
        // NOTE: 2020-05-20 15:02:02 désactivé le 20/05/2020 suite à réunion projet, demande Fred Brechon
        ...
        Ext.Ajax.request({
            ...
            callback: function (opt, success, oResponse) {
                ...
                if (oBack.success) {
                     ...
                     */
                    oMe._loadOfOper(oBack.liste[0].odf_id, false);
                } else {
                    var oMsg = Thot.app.MessageInfo();
                    oMsg.init(5000);
    ...
    onValidClick: function () {
        ...
        var aFormFields = {
            ...
            ala_id: Ext.getCmp('ala_id').getValue(),
        ...
        var oParams = {
            appName: Thot.app.appConfig.name,
            action: (oForm.param.idenreg == 0 ? 'NewAleaReglage' : 'updateSettingAct'),
            // on indique au backend que l'opération se fait en mode correction (pour la création)
            mode: 'correction',
            // paramètres du mode correction
            ...
            // paramètres pour création réglage en mode correction
            opn_id: Ext.getCmp('opn_id').getValue(),
            odf_id: Ext.getCmp('odf_id').getValue(),
            org_id: Ext.getCmp('org_id').getValue(),
            // commun
